\begin{frame}{Activité: Base-ball multicolore}
  ...
  \begin{block}{Règles du jeu (exemple à quatre équipes)}
    \begin{itemize}
      \item \structure{Installation :} disposer 4 bases autour du terrain et répartir 7 joueurs au hasard sur les bases (le joueur restant n'est pas utilisé).
      \item \structure{Coup autorisé :} déplacer un seul joueur à la fois, vers la base disposant d'une place libre, depuis une des deux bases voisines (interdit de traverser le terrain).
    \item  \structure{But :} Ramener tous les joueurs à leur base.
    \end{itemize}
  \end{block}
